...
output_dir = "YOUR_DATA_SET_DIRECTORY\\Waymo\\processed_data\\testing"
...
def preprocess_point_cloud(df_lidar, df_labels):
    ...
    labels = []
    # LiDARデータの前処理
    range_image_str = df_lidar['[LiDARComponent].range_image_return1.values'].values[0]
        ...
        for file in tqdm(files, desc=f"Processing files in {root}"):
            if file.endswith('.parquet'):
                ...
                if df_lidar is not None and df_labels is not None:
                    ...
                    print(f"Processed {len(point_clouds)} point clouds, {len(labels)} labels")
                    if point_clouds:
